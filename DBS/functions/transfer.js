...
exports = async function ({ query, headers, body }, response) {
  ...
  // Parse the body to extract payer, payee and amount
  const payer = parsedBody["payer"]; // 1449828371 //;
  const payee = parsedBody["payee"]; // 1725503673//;
  const amount = parsedBody["amount"]; // 1.2 //;
  
  // set the variables to store the values before the transaction block changes any values
  var resp = null;
  ...
  async function makeTransfer (session, payerColl, payeeColl) {
      ...
      // This can be done on DB Level using schema validation
